...
void phase1()
{
    ...
    for (int i = Time; i > 0; i--)
    {
        ...
        for (int k = 0; k < Task.size(); k++)
        {
            ...
            if (istaskavailable(arvl, deadlin, i))
            {
...
void remove_num_of_task(int &timeslot , int &count){
...
void removejobs(int &timeslot, double pow_to_reduce)
{
    ...
    remove_num_of_task(timeslot,count);
...
void phase2()
{
    ...
    while (itr < 80)
    {
      ...
      //  cout<<"avg_newtime "<<diff_with_avg_newtime<<" avg_oldtime "<<diff_with_avg_oldtime<<endl;
        double min_transferrable_pow = min({battery_capacity, diff_with_avg_newtime, diff_with_avg_oldtime});
       // cout<<"\n transferrable unit is "<<min_transferrable_pow<<endl;
        if (min_transferrable_pow > 1)
        {
            removejobs(timeslot, min_transferrable_pow);
            addjobs(newtimestamp,min_transferrable_pow);
